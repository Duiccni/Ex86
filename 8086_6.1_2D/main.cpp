...
LRESULT window_proc(HWND hwnd, UINT u_msg, WPARAM w_param, LPARAM l_param)
{
	...
	case WM_RBUTTONUP:
		...
		return 0;
	case WM_KEYUP:
		if (keyboard.update_key_up(w_param)) return 0;
		break;
	case WM_KEYDOWN:
		if (w_param == VK_F1) {
			if (PC - marked_pc < 5) {
		...
		if (w_param == VK_F11) {
			...
			else simulate = (simulate << 2) & 0b10100;
			return 0;
		}
		...
		if (keyboard.update_key_down(w_param)) return 0;
	}
...
void text_box() {
	graphics::draw::fill_rect({TB_L_X - 10, TB_T_Y + 20}, {TB_L_X + 1040 + 10, TB_T_Y - 500 + 20}, 0x181010, screen);
	u8 *ch = ram + 0xB8000;
	for (u8 y = 0; y < TEXT_MODE_Y; y++)
		for (u8 x = 0; x < TEXT_MODE_X; x++, ch += 2)
			font::unsafe_draw_char(ch[0], {TB_L_X + x * font::mfdim_xinc, TB_T_Y - y * 20}, get_color(~ch[1]), screen);
}
...

{
	...
	load_whole_file("bin/bootloader0.bin", floppies[0]);
	...
	load_whole_file("bin/EXPLORER3.bin", floppies[0] + SECTOR_SIZE * 4);
	load_whole_file("bin/FUN4.bin", floppies[0] + SECTOR_SIZE * 5);
	load_whole_file("bin/FUN5.bin", floppies[0] + SECTOR_SIZE * 6);
	load_whole_file("bin/FUN6.bin", floppies[0] + SECTOR_SIZE * 7);
