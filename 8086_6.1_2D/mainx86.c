...
u16 read_sectors_in() {
	...
	printf("\nRead from floppy%u:\n From: #%X * 512\n To: %X:%X\n Size: %u * 512\n", cpu.D.l, from, cpu.ES >> 4, cpu.B.x, cpu.A.l);
	from *= SECTOR_SIZE;
...
void get_key_in(u16 AX) {
	set_fbit(CFi, keyboard.get_key(AX));
}
